...
export default async function SetCard({
    ...
    return (
        <Link href={`/sets/${set.id}`} className="card p-4 hover:bg-card/50 transition cursor-pointer flex gap-3">
            <div className="rounded-full overflow-hidden size-fit">
                <Image
                    src={set.user.image || ""}
                    width={40}
                    height={40}
            ...
            <div>
                <div className="flex gap-3 items-center mb-3">
                    ...
                    <p className="rounded-full bg-card border-foreground/40 border-1 p-0.5 px-3 size-fit">{t(set.isPublic ? "public" : "private")}</p>
                </div>
